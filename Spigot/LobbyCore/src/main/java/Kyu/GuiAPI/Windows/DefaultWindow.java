...
public abstract class DefaultWindow implements Window {
    ...
    public GuiItem addItem(ItemStack item) {
        int slot = 0;
        int page = 1;
        while (getGuiItemFromPage(slot, page, false) != null) {
            slot++;
            if (slot > rows * 9 - 1) {
                slot = 0;
                page++;
     ...
     * @return GuiItem of Slot or null if slot is empty
     */
    public GuiItem getGuiItemFromPage(int slot, int page, boolean forceIgnorePageCheck) {
        if (!forceIgnorePageCheck) {
            checkMultiPage();
        }
        checkPageBounds(page);
        checkSlotBounds(slot);
    ...
    public AbstractMap.SimpleEntry<Integer, Integer> getNextFreeSlot(boolean allowCreateNewPage) {
        int slot = 0;
        int page = 1;
        while (getGuiItemFromPage(slot, page, true) != null) {
            slot++;
            if (slot > rows * 9 - 1) {
                slot = 0;
                page++;
    ...
    void removePlaceHolders(int page) {
        for (int i = 1; i != 3; i++) {
            GuiItem item = getGuiItemFromPage(rows * 9 - i, page, true);
    ...
    void doPlaceholderCheck(int slot, int page) {
        GuiItem item = getGuiItemFromPage(slot, page, true);
        if (item == null) return;
    ...
    void forceRemoveItem(int slot, int page) {
        GuiItem itemAtSlot = getGuiItemFromPage(slot, page, true);
